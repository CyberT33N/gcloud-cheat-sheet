...
- The command works only on Generic repositories and requires a role carrying the file delete permission on the repository (proven: `roles/artifactregistry.admin` carries it; the data-plane `roles/artifactregistry.writer` does not) — a governed cleanup therefore runs under the bounded admin wrapper, never under the standing data-plane grant.
