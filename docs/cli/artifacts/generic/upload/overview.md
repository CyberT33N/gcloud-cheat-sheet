...
- The upload requires a data-plane role carrying `artifactregistry.repositories.uploadArtifacts` on the repository (proven: [`roles/artifactregistry.writer`](../../../../iam/roles/predefined/artifactregistry/writer/overview.md) carries it); the IAM policy management around it follows the bounded wrapper form documented in the [IAM policy binding](../repositories/iam-policy-binding.md) card.
