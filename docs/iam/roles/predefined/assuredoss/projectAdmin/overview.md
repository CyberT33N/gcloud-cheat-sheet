...
| Property | Value |
| --- | --- |
| Role ID | `roles/assuredoss.projectAdmin` |
...
`roles/assuredoss.projectAdmin` grants 76 permissions across 6 services. Each service page lists exactly the permissions this role grants on that service and links every permission to its canonical page in the [central permission catalog](../../../../permissions/overview.md).
...
```shell
gcloud iam roles describe roles/assuredoss.projectAdmin --format="value(includedPermissions)"
```
