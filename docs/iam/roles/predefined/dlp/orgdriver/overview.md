...
`roles/dlp.orgdriver` grants 1676 permissions across 69 services. Each service page lists exactly the permissions this role grants on that service and links every permission to its canonical page in the [central permission catalog](../../../../permissions/overview.md).
...
```shell
gcloud iam roles describe roles/dlp.orgdriver --format="value(includedPermissions)"
```
