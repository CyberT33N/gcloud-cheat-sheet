...
| Property | Value |
| --- | --- |
| Role ID | `roles/discoveryengine.admin` |
...
`roles/discoveryengine.admin` grants 446 permissions across 11 services. Each service page lists exactly the permissions this role grants on that service and links every permission to its canonical page in the [central permission catalog](../../../../permissions/overview.md).
...
```shell
gcloud iam roles describe roles/discoveryengine.admin --format="value(includedPermissions)"
```
