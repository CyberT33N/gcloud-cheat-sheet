# `roles/firebase.admin`

Full access to Firebase products.

## Role details

| Property | Value |
| --- | --- |
| Role ID | `roles/firebase.admin` |
| Title | Firebase Admin |
| Type | Predefined role |
| Launch stage | `GA` |
| Included permissions | 782 |
| Service | [firebase](../overview.md) |

## Permissions

`roles/firebase.admin` grants 782 permissions across 56 services. Each service page lists exactly the permissions this role grants on that service and links every permission to its canonical page in the [central permission catalog](../../../../permissions/overview.md).

| Service | Permissions |
| --- | --- |
| [apikeys](permissions/apikeys/overview.md) | 4 |
| [appengine](permissions/appengine/overview.md) | 1 |
| [artifactregistry](permissions/artifactregistry/overview.md) | 32 |
| [automl](permissions/automl/overview.md) | 56 |
| [clientauthconfig](permissions/clientauthconfig/overview.md) | 8 |
| [cloudaicompanion](permissions/cloudaicompanion/overview.md) | 1 |
| [cloudasset](permissions/cloudasset/overview.md) | 1 |
| [cloudbuild](permissions/cloudbuild/overview.md) | 6 |
| [cloudconfig](permissions/cloudconfig/overview.md) | 2 |
| [cloudfunctions](permissions/cloudfunctions/overview.md) | 15 |
| [cloudkms](permissions/cloudkms/overview.md) | 5 |
| [cloudmessaging](permissions/cloudmessaging/overview.md) | 6 |
| [cloudnotifications](permissions/cloudnotifications/overview.md) | 1 |
| [cloudtestservice](permissions/cloudtestservice/overview.md) | 4 |
| [cloudtoolresults](permissions/cloudtoolresults/overview.md) | 14 |
| [databasesconsole](permissions/databasesconsole/overview.md) | 8 |
| [datastore](permissions/datastore/overview.md) | 52 |
| [errorreporting](permissions/errorreporting/overview.md) | 1 |
| [eventarc](permissions/eventarc/overview.md) | 86 |
| [fcmdata](permissions/fcmdata/overview.md) | 1 |
| [firebase](permissions/firebase/overview.md) | 18 |
| [firebaseabt](permissions/firebaseabt/overview.md) | 7 |
| [firebaseanalytics](permissions/firebaseanalytics/overview.md) | 2 |
| [firebaseappcheck](permissions/firebaseappcheck/overview.md) | 27 |
| [firebaseappdistro](permissions/firebaseappdistro/overview.md) | 6 |
| [firebaseapphosting](permissions/firebaseapphosting/overview.md) | 28 |
| [firebaseauth](permissions/firebaseauth/overview.md) | 11 |
| [firebasecrash](permissions/firebasecrash/overview.md) | 2 |
| [firebasecrashlytics](permissions/firebasecrashlytics/overview.md) | 7 |
| [firebasedatabase](permissions/firebasedatabase/overview.md) | 8 |
| [firebasedataconnect](permissions/firebasedataconnect/overview.md) | 35 |
| [firebasedynamiclinks](permissions/firebasedynamiclinks/overview.md) | 12 |
| [firebaseextensions](permissions/firebaseextensions/overview.md) | 4 |
| [firebaseextensionspublisher](permissions/firebaseextensionspublisher/overview.md) | 4 |
| [firebasehosting](permissions/firebasehosting/overview.md) | 5 |
| [firebaseinappmessaging](permissions/firebaseinappmessaging/overview.md) | 5 |
| [firebasemessagingcampaigns](permissions/firebasemessagingcampaigns/overview.md) | 7 |
| [firebaseml](permissions/firebaseml/overview.md) | 9 |
| [firebasenotifications](permissions/firebasenotifications/overview.md) | 5 |
| [firebaseperformance](permissions/firebaseperformance/overview.md) | 2 |
| [firebaserules](permissions/firebaserules/overview.md) | 11 |
| [firebasestorage](permissions/firebasestorage/overview.md) | 7 |
| [firebasevertexai](permissions/firebasevertexai/overview.md) | 8 |
| [logging](permissions/logging/overview.md) | 6 |
| [monitoring](permissions/monitoring/overview.md) | 9 |
| [oauthconfig](permissions/oauthconfig/overview.md) | 1 |
| [orgpolicy](permissions/orgpolicy/overview.md) | 1 |
| [recommender](permissions/recommender/overview.md) | 44 |
| [remotebuildexecution](permissions/remotebuildexecution/overview.md) | 1 |
| [resourcemanager](permissions/resourcemanager/overview.md) | 4 |
| [run](permissions/run/overview.md) | 69 |
| [runtimeconfig](permissions/runtimeconfig/overview.md) | 18 |
| [serviceusage](permissions/serviceusage/overview.md) | 14 |
| [stackdriver](permissions/stackdriver/overview.md) | 2 |
| [storage](permissions/storage/overview.md) | 66 |
| [storagebatchoperations](permissions/storagebatchoperations/overview.md) | 13 |

## Inspect this role live

```shell
gcloud iam roles describe roles/firebase.admin --format="value(includedPermissions)"
```

See [iam roles describe](../../../../../cli/iam/roles/describe/overview.md) for the verified command reference.

## Official documentation

- [IAM roles and permissions index](https://cloud.google.com/iam/docs/understanding-roles)
- [Roles overview](https://cloud.google.com/iam/docs/roles-overview)
