# `roles/firebase.analyticsViewer`

Read access to Google Analytics for Firebase.

## Role details

| Property | Value |
| --- | --- |
| Role ID | `roles/firebase.analyticsViewer` |
| Title | Firebase Analytics Viewer |
| Type | Predefined role |
| Launch stage | `GA` |
| Included permissions | 13 |
| Service | [firebase](../overview.md) |

## Permissions

`roles/firebase.analyticsViewer` grants 13 permissions across 5 services. Each service page lists exactly the permissions this role grants on that service and links every permission to its canonical page in the [central permission catalog](../../../../permissions/overview.md).

| Service | Permissions |
| --- | --- |
| [cloudnotifications](permissions/cloudnotifications/overview.md) | 1 |
| [firebase](permissions/firebase/overview.md) | 7 |
| [firebaseanalytics](permissions/firebaseanalytics/overview.md) | 1 |
| [firebaseextensions](permissions/firebaseextensions/overview.md) | 1 |
| [resourcemanager](permissions/resourcemanager/overview.md) | 3 |

## Inspect this role live

```shell
gcloud iam roles describe roles/firebase.analyticsViewer --format="value(includedPermissions)"
```

See [iam roles describe](../../../../../cli/iam/roles/describe/overview.md) for the verified command reference.

## Official documentation

- [IAM roles and permissions index](https://cloud.google.com/iam/docs/understanding-roles)
- [Roles overview](https://cloud.google.com/iam/docs/roles-overview)
