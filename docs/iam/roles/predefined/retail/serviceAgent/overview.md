...
| Property | Value |
| --- | --- |
| Role ID | `roles/retail.serviceAgent` |
...
`roles/retail.serviceAgent` grants 67 permissions across 9 services. Each service page lists exactly the permissions this role grants on that service and links every permission to its canonical page in the [central permission catalog](../../../../permissions/overview.md).
...
```shell
gcloud iam roles describe roles/retail.serviceAgent --format="value(includedPermissions)"
```
